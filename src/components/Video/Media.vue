  ...
  <div>
    <div class="w-full flex justify-center items-center w-full bg-black">
      <div class="relative" @mouseleave="()=>{this.isTool = false}">
        <div
          class="absolute flex cursor-pointer z-50 items-center justify-center h-8 w-8 bg-forest rounded-full left-2 top-2 rounded"
          :class="isTool?`visible`:`invisible`">
        ...
        <div class="absolute w-full bottom-4 left-0 w-full px-10" :class="isTool?`visible`:`invisible`">
          <div class="h-10 w-full bg-black bg-opacity-70 rounded-md flex items-center px-5">
            ...
            <div class="cursor-pointer mr-5 relative" @click="()=>{this.isSetting =! this.isSetting}">
              ...
              <div class="absolute bg-black text-white rounded w-36 bottom-9 pt-1 bg-opacity-90" v-if="isSetting">
                <ul>
                  ...
                  <li class="flex items-center justify-between px-3 py-2 border-b border-gray-700"
                      @click="selectQuality(`360p`)">
                    <span>360</span>
            ...
            <div class="flex itesm-center justify-center ml-5 w-56">
              <input id="volumeSlider" type="range" min="0" max="100" v-model="audioVolume" step="1"
                     @change="setVolume($event)" class="w-full">
            </div>
...
export default {
  ...
  methods: {
    ...mapActions('course', ['setLastWatch']),
    selectQuality (qty) {
      this.quality = qty
      ...
      let interval = setInterval(() => {
        if (document.getElementById('Moeys') !== null) {
          ...
          if (this.timeMin != null) {
            this.vid.currentTime = this.timeMin
          }
