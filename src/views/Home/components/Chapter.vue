<template>
  <div class="fixed z-50 w-full h-full top-0 left-0 bg-black bg-opacity-90 flex items-center justify-center">
    ...
    <div v-else class="bg-white rounded-md shadow w-96 text-black text-sm relative">

      <div class="relative z-50">
        <div class="text-base font-semibold w-full border-b h-12 flex items-center pl-5 justify-between relative z-50">
          <div class="cursor-pointer" @click="back">
            <BackIcon></BackIcon>
          </div>
          <div class="text-center w-full">{{ title }}</div>
        </div>
        <div class="py-10 left-0 top-0 w-full h-full flex items-center justify-center" v-if="chapters.length <= 0">
          <Empty></Empty>
        </div>
        ...
        <ul class="px-5 max-h-96 overflow-y-scroll pb-5">
          <li v-for="(chapter, index) in chapters" :key="index" class="py-2 cursor-pointer"
            ...
            <div class="flex items-center bg-forest rounded-md px-3 py-2">
              <div :style="{backgroundColor:`${chapter.chapterColor}`}"
