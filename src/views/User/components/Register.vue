  ...
  <div class="fixed w-full h-full flex items-center justify-center flex-col z-50 login py-10">
    ...
    <div class="w-99 p-5" v-if="!stepTwo">
      ...
      <input type="text" ref="firstName"
             ...
             placeholder="នាមត្រកូល" v-model="user.first_name"/>
      <div class="mb-5"></div>
      <input type="text" v-model="user.last_name" ref="lastName"
             class="h-10 w-full rounded-full bg-transparent border border-white px-3 text-sm placeholder-white outline-none"
      ...
      <div>
        <label class="flex items-center text-xs mt-5 cursor-pointer" @click="()=>{this.isTerm = true}">
          <div class="h-4 w-4 border border-white rounded-full mr-2 flex items-center justify-center">
            <div class="bg-white h-2 w-2 bg-white rounded-full" v-if="user.term"></div>
          </div>
          <p>
            តើអ្នកយល់ព្រមជាមួយ<input type="checkbox" v-model="user.term" class="hidden"/><span
            class="text-secondary cursor-pointer">លក្ខខ័ណ្ណ </span> នឹងការបង្កើតនិងប្រើប្រាស់គណនី?
          </p>
        </label>
      </div>
      <button class="relative rounded-full bg-white text-primary w-full h-10 mt-5 flex items-center justify-center"
              @click="createAccount">
    ...
    <div class="w-99 p-5 text-sm" v-else>
      ...
      <div class="relative">
        <input type="date" ref="dob" v-model="registerNextStep.dob" id="calendar"
               class="mt-5 w-full h-10 rounded-full outline-none px-5 border border-white bg-transparent text-white">
      </div>
      <div class="mt-5">
        <div class="h-10 w-full rounded-full border border-white flex items-center px-5 justify-between"
             @click="()=>{this.isProvince = true}">
          <div>
      ...
      <div class="flex justify-end items-center mt-5 text-sm" v-if="showType">
        <label class="flex items-center">
          <input type="radio" name="type_id" v-model="registerNextStep.type_id" value="1" class="hidden">
          <div class="w-4 h-4 rounded-full border-white border flex items-center justify-center">
            <div class="rounded-full h-3 w-3 bg-white" v-if="registerNextStep.type_id == 1"></div>
          </div>
          <div class="ml-3">ថ្នាក់វិទ្យាសាស្រ្ត</div>
        </label>
        <div class="mx-3"></div>
        <label class="flex items-center">
          <input type="radio" name="type_id" v-model="registerNextStep.type_id" value="2" class="hidden">
          <div class="w-4 h-4 rounded-full border-white border flex items-center justify-center">
            <div class="rounded-full h-3 w-3 bg-white" v-if="registerNextStep.type_id == 2"></div>
          </div>
          <div class="ml-3">ថ្នាក់វិទ្យាសាស្រ្តសង្គម</div>
        </label>
      </div>
      <button
        class="relative rounded-full bg-white text-primary font-semibold w-full h-10 mt-5 flex items-center justify-center"
        @click="nextStep">
        <div class="absolute -top-3" v-if="loading">
          <LoaderIcon></LoaderIcon>
        </div>
    ...
    <template v-if="isSchool">
      <School @back="closeSchool" @selectedSchool="selectedSchool($event)"
              :province_id="registerNextStep.pro_id"></School>
    </template>
